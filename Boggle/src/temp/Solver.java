    ...
    public void solve(Node node, Trie root) {
        ...
        while (!path.isEmpty()) {
            ...
            if (!inStack(child)) {
                ...
                //but first check if letter is a 'Q', as 'Q' in boggle
                //is to always be interpreted as 'Qu'
                if (child.value == Character.toUpperCase('Q')) {
                    Trie qNode = current.trie.getChild(child.value);
                    path.push(new Tuple(child, qNode.getChild('U')));
                }
                else path.push(new Tuple(child, current.trie.getChild(child.value)));
